...
from plan_data import PLAN_DATA
import re
import os
...
def generate_course_syllabus(filename="COURSE_Syllabus.pdf"):
    ...
    story.append(Paragraph("Course Syllabus & Roadmap", STYLES['SubTitle']))
...
def generate_module_overview(module, filename):
    ...
    for week in module['weeks']:
        ...
        for d in week['days']:
            ...
            theory_clean = d['theory'].replace('<br/>', ' ').replace('\n', ' ')
            if len(theory_clean) > 80: theory_clean = theory_clean[:77] + "..."
            
            practice_clean = d['practice'].replace('<br/>', ' ').replace('\n', ' ')
            ...
            row = [
                ...
                Paragraph(linkify(theory_clean), STYLES['CellText']),
                Paragraph(linkify(practice_clean), STYLES['CellText']),
...
def generate_daily_detailed(day_data, filename, week_info="", module_info=""):
    ...
    story.append(Paragraph(linkify(theory_text), STYLES['ContentText']))
    ...
    story.append(Paragraph(linkify(practice_text), STYLES['ContentText']))
...
if __name__ == "__main__":
    ...
    generate_course_syllabus("00_Course_Syllabus.pdf")
    
    # 2. Generate Module Overviews
    for i, module in enumerate(PLAN_DATA['modules']):
        filename = f"0{i+1}_Module_{i+1}_Overview.pdf"
        generate_module_overview(module, filename)
        
    # 3. Generate detailed Daily PDFs for Week 1 (Days 1-7)
    # Browse through modules to find Week 1
    for module in PLAN_DATA['modules']:
        for week in module['weeks']:
            if week['week_num'] == 1:
                for day in week['days']:
                    # Clean up filename (spaces to underscores, no special chars)
                    clean_topic = re.sub(r'[^a-zA-Z0-9]', '_', day['topic'])
                    clean_topic = re.sub(r'_+', '_', clean_topic).strip('_')
                    
                    filename_day = f"Day_{day['day'].split(' ')[1].zfill(3)}_{clean_topic}.pdf"
